...
public class AnimateTiledTexture : MonoBehaviour
{
	...
	private Dictionary<string, KeyValuePair<int, int> > animations = new Dictionary<string, KeyValuePair<int, int>>();
 	private int playFromIndex = 0;
	private int playToIndex = 3;
	...
	public void RegisterAnimation(string name, int indexFrom, int indexTo)
	{
		animations.Add(name, new KeyValuePair<int, int>(indexFrom, indexTo));
	}
	...
	{
		KeyValuePair<int, int> fromTo = animations[name];
		playFromIndex = fromTo.Key;
		playToIndex = fromTo.Value;
...
//		Play();
    ...
    {
		if(_index < playFromIndex || _index > playToIndex )
			_index = playFromIndex;
        ...
        Vector2 offset = new Vector2((float)_index / _columns - (_index / _columns), //x index
                                      1f - ((_index / _columns) / (float)_rows));    //y index
